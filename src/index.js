...
window.onload = () => {
    ...
    const view = new EditorView({
        state: EditorState.create({
            extensions: [basicSetup, rich(), theme.of(EditorView.theme(generateTheme(darkmode), {
    ...
    lintButton.addEventListener("click", () => {
        ...
        const lines = [...view.state.doc].join("").replace(/&/g, "&amp;").replaceAll(/<noparse>(.+)<\/noparse>/g, (_match, name) => {
            return escapeHTML(name);
        }).replaceAll(lookbehind, (match, name, attr) => {
            switch (name) {
            case "color": {
                if (!attr || !(supportedColors.includes(attr) ||
                        ((attr.length === 7 || attr.length === 9) && attr.startsWith("#") && !isNaN(parseInt(attr.substring(1), 16))))) { //ugly
            ...
            case "sprite": {
                const number = parseInt(attr);
                if (!isNaN(number) && spriteList[number]) {
            ...
            /*case "line-height":
                if (!isNaN(attr)) {
                    ...
                    return `<span style="margin-bottom: ${num}px">`;
                } else if (checkUnits(["px", "em", "%"], attr)) {
                    return `<span style="margin-bottom: ${attr}">`;
                }
                return escapeHTML(match);*/
            case "u":
            case "b":
